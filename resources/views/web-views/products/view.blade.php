...
@push('css_or_js')
    <meta property="og:image" content="{{Storage::url('/company')}}/{{$web_config['web_logo']}}"/>
    <meta property="og:title" content="Products of {{$web_config['name']}} "/>
    <meta property="og:url" content="{{env('APP_URL')}}">
    <meta property="og:description" content="{!! substr($web_config['about']->value,0,100) !!}">

    <meta property="twitter:card" content="{{Storage::url('/company')}}/{{$web_config['web_logo']}}"/>
...
@php($decimal_point_settings = \App\CPU\Helpers::get_business_settings('decimal_point_settings'))
    ...
    <div class="d-flex w-100 justify-content-center align-items-center mb-3 __min-h-70px __inline-35" style="background:{{$web_config['primary_color']}}10;">
        ...
        <div class="text-capitalize container">
            <span class="__text-18px font-semibold">{{str_replace("_"," ",$data['data_from'])}} {{\App\CPU\translate('products')}} {{ isset($brand_name) ? '('.$brand_name.')' : ''}}</span>
        </div>
