                                    ...
                                    @foreach($unique_shops as $key=>$shop)
                                        <div class="chat_list @if ($key == 0) btn--primary @endif"
                                            ...
                                            <div class="chat_people" id="chat_people">
                                                <div class="chat_img">
                                                    <img onerror="this.src='{{asset('public/assets/front-end/img/image-place-holder.png')}}'" src="{{ $shop->delivery_man_id ?Storage::url('/delivery-man/'.$shop->image) : Storage::url('/shop/'.$shop->image)}}"
                                                        class="__rounded-10">
                                                </div>
                                                <div class="chat_ib">
                                                ...
                                                @if ($chat->sent_by_seller? $chat->sent_by_seller : $chat->sent_by_delivery_man)
                                                    <div class="incoming_msg">
                                                        <div class="incoming_msg_img"><img
                                                                src="@if($chat->image == 'def.png'){{Storage::url('/'.$chat->image)}} @else {{ $shop->delivery_man_id ?Storage::url('/delivery-man/'.$last_chat->delivery_man->image) : Storage::url('/shop/'.$last_chat->shop->image)}}
                                                                @endif"
                                                                alt="sunil"></div>
                        ...
                        if (data.length != 0) {
                            data.map((element, index) => {
                                ...
                                if (element.sent_by_customer) {
                                    ...
                                    let img_path = element.image == 'def.png' ? `{{ Storage::url('/shop') }}/${element.image}` : `{{ (isset($shop->delivery_man_id) && $shop->delivery_man_id) ? Storage::url('/delivery-man') : Storage::url('/shop') }}/${element.image}`;
