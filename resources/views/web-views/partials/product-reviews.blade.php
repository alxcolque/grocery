        ...
        <div
            ...
            <div
                ...
                <img class="rounded-circle __img-64 object-cover"
                    ...
                    src="{{Storage::url("/profile")}}/{{(isset($productReview->user)?$productReview->user->image:'')}}"
        ...
        <div class="col-md-7">
            ...
            @if (!empty(json_decode($productReview->attachment)))
                @foreach (json_decode($productReview->attachment) as $key => $photo)
                    <img onclick="showInstaImage('{{Storage::url("/review/$photo")}}')" class="cz-image-zoom __img-70 rounded border" onerror="this.src='{{asset('public/assets/front-end/img/image-place-holder.png')}}'" src="{{Storage::url("/review/$photo")}}" alt="Product review">
                @endforeach
            @endif
        </div>
