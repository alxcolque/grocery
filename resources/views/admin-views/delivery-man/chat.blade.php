                    ...
                    <div class="card card-body px-0 h-100">
                        <div class="media align-items-center px-3 gap-3 mb-4">
                            <div class="avatar avatar-sm avatar-circle">
                                <img class="avatar-img" onerror="this.src='{{ asset('public/assets/front-end/img/image-place-holder.png') }}'" src="{{Storage::url('/seller/')}}/{{auth('admin')->user()->image}}" alt="Image Description">
                                <span class="avatar-status avatar-sm-status avatar-status-success"></span>
                            </div>
                            <div class="media-body">
                                <h5 class="profile-name mb-1">{{ auth('admin')->user()->name }}</h5>
                                <span class="fz-12">{{\App\CPU\translate('super_admin')}}</span>
                            </div>
                        </div>
                            ...
                            <form class="search-form px-3" id="chat-search-form">
                                <div class="search-input-group">
                                    ...
                                    <input
                                        ...
                                        placeholder="{{\App\CPU\translate('Search_delivery_man...')}}"
                            ...
                            <div class="inbox_chat d-flex flex-column mt-1">
                                @foreach($chattings_user as $key => $chatting)
                                    <div class="list_search">
                                        <div class="chat_list p-3 d-flex gap-2 user_{{$chatting->delivery_man_id}} seller-list @if ($key == 0) active @endif"
                                            ...
                                            <div class="chat_people media gap-10" id="chat_people">
                                                <div class="chat_img avatar avatar-sm avatar-circle">
                                                    <img src="{{ Storage::url('/delivery-man/'.$chatting->image) }}" id="{{$chatting->delivery_man_id}}" onerror="this.src='{{asset('public/assets/front-end/img/image-place-holder.png')}}'" class="avatar-img avatar-circle">
                                                    <span class="avatar-status avatar-sm-status avatar-status-success"></span>
                                                </div>
                    ...
                    <div class="card card-body card-chat justify-content-between Chat">
                        ...
                        <div class="inbox_msg_header d-flex flex-wrap gap-3 justify-content-between align-items-center border px-3 py-2 rounded mb-4">
                            ...
                            <div class="media align-items-center gap-3">
                                <div class="avatar avatar-sm avatar-circle">
                                    <img class="avatar-img" id="profile_image" src="{{ Storage::url('/delivery-man/'.$chattings_user[0]->image) }}" onerror="this.src='{{ asset('public/assets/front-end/img/image-place-holder.png') }}'" alt="Image Description">
                                    <span class="avatar-status avatar-sm-status avatar-status-success"></span>
                                </div>
                                <div class="media-body">
...
@push('script')
    <script>
        function messageGet(e){
            ...
            let url ="{{ route('admin.delivery-man.ajax-message-by-delivery-man') }}" +"?delivery_man_id=" + user_id;
        ...
        $(document).ready(function () {
            ...
            $("#msgSendBtn").click(function (e) {
                ...
                var delivery_man_id = $('#hidden_value').val();
                ...
                $.ajax({
                    type: "post",
                    url: "{{ route('admin.delivery-man.ajax-admin-message-store') }}",
                    data: data,
