                                ...
                                <div class="d-flex flex-column gap-2 mt-3">
                                    ...
                                    <div class="order-status d-flex justify-content-sm-end gap-10 text-capitalize">
                                        ...
                                        @elseif($order['order_status']=='failed')
                                            <span class="badge badge-danger font-weight-bold radius-50 d-flex align-items-center py-1 px-2">{{str_replace('_',' ',$order['order_status'] == 'failed' ? 'Failed to Deliver' : '')}}
                                            </span>
                                                        ...
                                                        <div class="modal-body">
                                                            @if($detail->digital_file_after_sell)
                                                                <div class="mb-4">
                                                                    ...
                                                                    <a href="{{ Storage::url('/product/digital-product/'.$detail->digital_file_after_sell) }}"
            ...
            <div class="col-lg-4 col-xl-3 d-flex flex-column gap-3">
                <div class="card">
                    <div class="card-body text-capitalize d-flex flex-column gap-4">
                        ...
                        <div class="">
                            <label class="font-weight-bold title-color fz-14">{{\App\CPU\translate('Payment_Status')}}</label>
                            ...
                            <div class="media flex-wrap gap-3">
                                <div class="">
                                    <img class="avatar rounded-circle avatar-70"
                                         ...
                                         src="{{Storage::url('/profile/'.$order->customer->image)}}"
                                         alt="Image">
                                </div>
                                <div class="media-body d-flex flex-column gap-1">
                                    <span class="title-color"><strong>{{$order->customer['f_name'].' '.$order->customer['l_name']}} </strong></span>
                                    <span class="title-color"> <strong>{{\App\Model\Order::where('customer_id',$order['customer_id'])->count()}}</strong> {{\App\CPU\translate('orders')}}</span>
                        ...
                        <div class="media">
                            @if($order->seller_is == 'admin')
                                <div class="mr-3">
                                    <img class="avatar rounded avatar-70" onerror="this.src='https://6valley.6amtech.com/public/assets/front-end/img/image-place-holder.png'"
                                         src="{{Storage::url("/company/$company_web_logo")}}" alt="">
                                </div>
                            ...
                            @else
                                @if(!empty($order->seller->shop))
                                    <div class="mr-3">
                                        <img class="avatar rounded avatar-70" onerror="this.src='https://6valley.6amtech.com/public/assets/front-end/img/image-place-holder.png'"
                                             src="{{Storage::url('/shop')}}/{{$order->seller->shop->image}}" alt="">
                                    </div>
                                    <div class="media-body d-flex flex-column gap-2">
    ...
    <script>
        ...
        function initializegLocationMap() {
            ...
            google.maps.event.addListener(marker, 'click', (function (marker) {
                return function () {
                    infowindow.setContent("<div class='float-left'><img class='__inline-5' src='{{Storage::url('/profile/')}}{{$order->customer->image??""}}'></div><div class='float-right __p-10'><b>{{$order->customer->f_name??""}} {{$order->customer->l_name??""}}</b><br/>{{$shipping_address->address??""}}</div>");
